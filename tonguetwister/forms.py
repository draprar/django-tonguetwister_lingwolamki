...
class CustomUserCreationForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        if commit:
            user.save()
            regular_users_group = Group.objects.get(name='Regular Users')  # Automatically assign user to 'Regular Users' group after saving
...
# Contact form with fields for name, email, and a message
class ContactForm(forms.Form):
    name = forms.CharField(max_length=50)  # Field for the user's name
    email = forms.EmailField()  # Field for the user's email
    message = forms.CharField(widget=forms.Textarea)  # Text area for the message
